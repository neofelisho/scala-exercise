...
class TrieMapOrderServiceTest extends AnyPropSpec with Matchers with ScalaCheckDrivenPropertyChecks {

  property("should start with empty store") {
    val service = new TrieMapOrderService[Int, Int, Long]()
  ...
  property("can add bulk items in parallel") {
    forAll(Gen.listOfN(10, Gen.choose(1, 100))) { list =>
      val service = new TrieMapOrderService[Int, String, Long]()
      list
        .zipWithIndex
        .map {
          case (n, idx) =>
